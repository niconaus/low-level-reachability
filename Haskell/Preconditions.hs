...
tauB (Jump l) blocks p = case M.lookup l blocks of
  Nothing -> error "jump label to non-existing block"
  Just b -> junction [tauB b blocks p]
tauB (IndirectJump e) blocks p = junction [ tauB b blocks p `restrict` Expr (App (Num l) Eq e) | (l,b) <- M.toList blocks]
tauB (CJump e l1 l2) blocks p = case (M.lookup l1 blocks, M.lookup l2 blocks) of
...
tauStore :: E -> V -> Predicate -> [Predicate]
...
tauStoreE :: E -> V -> E -> [(E,Predicate)]
tauStoreE _ _    (Num n) = [(Num n, Expr true)]
tauStoreE _ _    (Var x) = [(Var x, Expr true)]
tauStoreE r v (Deref  a) = [(Deref a , Expr $ App a Eq r),
                            (Var v   , Expr $ Not (App a Eq r))]
