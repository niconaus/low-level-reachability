...
import Data.List
    ( group, insertBy, intercalate, partition, sort, sortBy )
...
instance Treeish [[Predicate]] where
  ...
  extend f (ps:pt) = foldr (zipWithOpt (++) . f) ([]:extend f pt) ps
  extend _ [] = []
...
instance Show Fangorn where
  show = Data.List.intercalate "\n" . preShow
    where preShow :: Fangorn -> [String]
          preShow (Fangorn ps trees) = (case ps of [] -> "_"; _ -> show (foldr1 PAnd ps)) : (map prefix.concatMap preShow) trees
          prefix s@('↳':_) = '\t':s
          prefix s@('\t':_) = '\t':s
          prefix s = '↳':'\t':s
...
mapPT f (Node ps pt) = foldr (mergeTrees . f) (Node [] (mapPT f pt)) ps
mapPT _ Empty = Empty
...
predTreeAnd (Node preds tree) e = Node (map (`PAnd` e) preds) (predTreeAnd tree e)
...
eSubst v e expr = if v == expr then e else
                  case expr of Not e1 -> Not (eSubst v e e1)
                               ...
                               Deref e1 -> Deref (eSubst v e e1)
                               _ -> expr

predSubst :: E -> E -> Predicate -> Predicate
predSubst v e (Exists mv p)
  | v == Var mv = Exists mv p
  | isBoundBy mv (Expr e) = let mv' = mv++"'" in predSubst v e (Exists mv' (predSubst (Var mv) (Var mv') p))
  | otherwise = Exists mv (predSubst v e p)
...
isBoundBy bv pr = not (null [undefined | Var v <-atoms pr, v==bv])
  where
    ...
    atoms (Expr (Deref e)) = atoms (Expr e)
...
simp p@(PAnd _ _)
  ...
  where conj = subsume . rewriteEq . map simp' . unravel
        ...
        unravel (Exists mv p1) = let (bound, unbound) = partition (isBoundBy mv) (conj p1) in unbound ++ ([Exists mv (foldr1 PAnd bound) | not (null bound)])
        unravel p1 = [p1]
...
simpE (App e1 op e2) = perform op (simpE e1, simpE e2)
  where
    ...
    perform Minus (e@(App x Minus y),z)
      | y==z = App x Minus (App (Num 2) Mult y)
      | e==z = Num 0
      | otherwise = App e Minus z
    perform Minus (e@(App x Plus y),z)
      | y==z = x
      | e==z = Num 0
      | otherwise = App e Minus z
    ...
    perform Or (x, y)
      ...
      | y==Not true = x
      | x==true || y==true = true
      | otherwise = App x Or y
    perform And (x, y)
      | x==true = y
      | y==true = x
      | x==Not true || y==Not true = Not true
      | otherwise = App x And y
    perform op' (x, y) = App x op' y

simpE (Deref e) = Deref  (simpE e)
...
rewriteEq ps = map Expr (subsume equalities) ++ map (foldl (.) simp $ map (uncurry predSubst) equalities) ps
 where
   equalities = [ if x >= y then (x,y) else (y,x) | App x Eq y <- trans $ [ineq e | Expr e <- ps] ]
   ...
   trans = foldr (\e es-> (case e of
                           ...
                           App x Leq y -> if App y Leq x `elem` es then App x Eq y else e
                           Not (App x Leq y) -> if Not (App y Leq x) `elem` es then App (Num 0) Eq (Num 1) else e {- generate an absurdity -}
                           _ -> e):es) []
