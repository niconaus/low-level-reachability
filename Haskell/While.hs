...
jlang whilecode = link jumpcode
 where
  ...
  compile (While e b) n cont = [(l+2)==>CJump e l (l+1), (l+1)==>cont] ++ body :§ loc
    where body@((l,_):_) :§ loc = compile b n (CJump e l (l+1))
  compile (If e thn els) n cont = [(l'+2)==>CJump e l l', (l'+1)==>cont] ++ thn' ++ els' :§ loc1++loc2
    where thn'@((l,_):_)  :§ loc1 = compile thn n     (Jump (l'+1))
          els'@((l',_):_) :§ loc2 = compile els (l+1) (Jump (l'+1))
  ...
  compile (b `Until` e) n cont = [(l+2)==>Jump l, (l+1)==>cont] ++ body :§ loc
    where body@((l,_):_) :§ loc = compile b n (CJump e (l+1) l)

  compile (Goto l) n _ = let k = labels M.! l in [n==>Jump k] :§ []
  compile (Label label w) n cont = [(l+1)==>Jump l] ++ body :§ (label,l):loc
